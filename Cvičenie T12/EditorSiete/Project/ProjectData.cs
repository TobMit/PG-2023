...
namespace EditorSiete.Project
{
    ...
    public static class ProjectData
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(NetworkData.BitmapPath))
                return false;
            ...
            try
            {
                ...
                Bitmap = new Bitmap(NetworkData.BitmapPath);

                CoordTransformations.Umin = 0;
                CoordTransformations.Vmax = 0;
                CoordTransformations.Umax = Bitmap.Width;
        ...
        {
            for (int i = NetworkData.Edges.Count-1; i >= 0; i--)
            {
                if (NetworkData.Edges[i].StartNode == node || NetworkData.Edges[i].EndNode == node)
        ...
        public static void LoadNetworkData()
        {
            ...
            {
                NetworkNode? startNode = GetNetworkNodes().Find(n => n.ID == edge.StartNodeId);
                NetworkNode? endNode = GetNetworkNodes().Find(n => n.ID == edge.EndNodeId);
                
                if (startNode == null || endNode == null)
                    continue;

                GetNetworkEdges().Add(new NetworkEdge(startNode, endNode));
            }

            NetworkData.BitmapPath = data.BitmapPath;
            NetworkData.BackgroundVisible = data.BackgroundVisible;
