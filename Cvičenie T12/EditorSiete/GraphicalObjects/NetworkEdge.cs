    ...
    public class NetworkEdge : IDrawable2DObject
    {
        ...
        { 
            get
            {
                ...
                var startPointXY = CoordTransformations.FromUVtoXY(startNode.Position);
                var endPointXY = CoordTransformations.FromUVtoXY(endNode.Position);

                SizeF distance = new(startPointXY.X - endPointXY.X, startPointXY.Y - endPointXY.Y);

                return (float)Math.Sqrt(distance.Width * distance.Width + distance.Height * distance.Height);
            }
        }
