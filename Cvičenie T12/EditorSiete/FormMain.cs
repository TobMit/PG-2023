...
namespace EditorSiete
{
    public partial class FormMain : Form
    {
        ...
        public FormMain()
        {
            ...
            bezierCurve = new();
            bezierCurve.Add(new Matrix(new float[,] { { -200, -200, 1 } }, transpose: true));
            bezierCurve.Add(new Matrix(new float[,] { { 290, 100, 1 } }, transpose: true));
            bezierCurve.Add(new Matrix(new float[,] { { -150, 20, 1 } }, transpose: true));
            bezierCurve.Add(new Matrix(new float[,] { { -300, 250, 1 } }, transpose: true));
            bezierCurve.Add(new Matrix(new float[,] { { 200, 200, 1 } }, transpose: true));

            airplane = new();
            airplane.Transform(Matrix.GetIdentityMatrix(3));
        }
        ...
        private void doubleBufferedPanelDrawing_Paint(object sender, PaintEventArgs e)
        {
            ...
            //foreach(Abscissa2D line in lines)
            //{
            //    line.Draw(g);
            //}
            ...
            float angle = 0;
            Matrix point = bezierCurve.GetPointAndAngleOnCurve(timeOnCurve, out angle);

            Matrix scale = new(new float[,] { { 5, 0, 0 }, { 0, 2, 0 }, { 0, 0, 1 } });
            Matrix rotate = new(new float[,] { { MathF.Cos(angle), -MathF.Sin(angle), 0 }, { MathF.Sin(angle), MathF.Cos(angle), 0 }, { 0, 0, 1 } });
            Matrix translate = new(new float[,] { { 1, 0, 0 }, { 0, 1, 0 }, { point[0, 0], point[1, 0], 1 } }, transpose: true);
            Matrix final = translate * rotate * scale;
            airplane.Transform(final);
        ...
        private void timerAnimation_Tick(object sender, EventArgs e)
        {
            ...
            long x = sw.ElapsedMilliseconds % 5000;

            timeOnCurve = x / 5000.0f;
